...
int RecognizeNumberValue(std::string s) {
    ...
    // TO BE DONE
    int num_temp;
    if(s[0] == 'B') return std::stoi(s.substr(1), 0, 2);
    if(s[0] == 'X') return std::stoi(s.substr(1), 0, 16);
...
std::string NumberToAssemble(const int &number) {
    ...
    for( ;num >= 2;num = num / 2) {
        if(num % 2 == 1) BinNumString = "1" + BinNumString;
        else BinNumString = "0" + BinNumString;
    }
    BinNumString = "1" + BinNumString;

    for(int i = BinNumString.size(); i < 16 ; i++) {
...
std::string NumberToAssemble(const std::string &number) {
    ...
    std::string BinNumString = NumberToAssemble(num);
    return BinNumString;
}
...
std::string ConvertBin2Hex(std::string bin)  {
    ...
    // TO BE DONE
    std::string HexNumString;
    int firstpos;
    for(firstpos = 0;firstpos < 16; firstpos += 4) {
        std::string SubBinstring="";
        SubBinstring = bin.substr(firstpos, 4);
        if(SubBinstring == "0000") HexNumString = HexNumString + "0";
        if(SubBinstring == "0001") HexNumString = HexNumString + "1";
        if(SubBinstring == "0010") HexNumString = HexNumString + "2";
        if(SubBinstring == "0011") HexNumString = HexNumString + "3";
        if(SubBinstring == "0100") HexNumString = HexNumString + "4";
        if(SubBinstring == "0101") HexNumString = HexNumString + "5";
        if(SubBinstring == "0110") HexNumString = HexNumString + "6";
        if(SubBinstring == "0111") HexNumString = HexNumString + "7";
        if(SubBinstring == "1000") HexNumString = HexNumString + "8";
        if(SubBinstring == "1001") HexNumString = HexNumString + "9";
        if(SubBinstring == "1010") HexNumString = HexNumString + "A";
        if(SubBinstring == "1011") HexNumString = HexNumString + "B";
        if(SubBinstring == "1100") HexNumString = HexNumString + "C";
        if(SubBinstring == "1101") HexNumString = HexNumString + "D";
        if(SubBinstring == "1110") HexNumString = HexNumString + "E";
        if(SubBinstring == "1111") HexNumString = HexNumString + "F";
    }
    return HexNumString;
}
...
std::string assembler::TranslateOprand(int current_address, std::string str, int opcode_length) {
    ...
    if (!(item.getType() == vAddress && item.getVal() == -1)) {
        ...
        std::string wholenum = NumberToAssemble(offset);
        return wholenum.substr(16 - opcode_length, opcode_length);

    }
    if (str[0] == 'R') {
        // str is a register
        // TO BE DONE
        int register_index = RecognizeNumberValue(str.substr(1));
        if (register_index > 7 || register_index < 0) {
            //illegal register
            std::cout << "-8" << std::endl;
            exit(-8);
        }
        else return NumberToAssemble(register_index).substr(13, 3);

    } else {
        // str is an immediate number
        // TO BE DONE
        
        if (str.find_first_of("XB#") == 0) {
            if (str[0] == 'X') {
                if (str.substr(1).find_first_not_of("-0123456789ABCDEF") != std::string::npos) {
                    //unknown label
                    std::cout << "-9" << std::endl;
                    exit(-9);
                }
            ...
            if (str[0] == 'B') {
                if (str.substr(1).find_first_not_of("-01") != std::string::npos) {
                    //unknown label
                    std::cout << "-9" << std::endl;
                    exit(-9);
                }
        ...
        int num_temp = RecognizeNumberValue(str);

        if (opcode_length == 5 && (num_temp > 15 || num_temp < -16)) {
            std::cout << "-7" << std::endl;
            exit(-7);
        }

        if (opcode_length == 6 && (num_temp > 31 || num_temp < -32)) {
            std::cout << "-7" << std::endl;
            exit(-7);
        }

        return NumberToAssemble(num_temp).substr(16 - opcode_length, opcode_length);
    }
}
...
int assembler::assemble(std::string input_filename, std::string output_filename) {
    ...
    if (input_file.is_open()) {
        ...
        while (std::getline(input_file, line)) {
            ...
            // TO BE DONE
            int lstringpos = line.find_first_of('\"');
            int rstringpos = line.find_last_of('\"');
            if (lstringpos != std::string::npos && rstringpos != std::string::npos && lstringpos != rstringpos) {
                std::string lpart = line.substr(0, lstringpos);
                transform(lpart.begin(), lpart.end(), lpart.begin(), ::toupper);

                std::string midpart = line.substr(lstringpos, rstringpos);

                std::string rpart = line.substr(rstringpos + 1);
                transform(rpart.begin(), rpart.end(), rpart.begin(), ::toupper);
                line = lpart + midpart + rpart;
            }
            else {
    ...
    for (int line_index = 0; line_index < file_content.size(); ++line_index) {
        ...
        if (line[0] == '.') {
            ...
            if (pseudo_command == ".ORIG") {
                ...
                if (stringword.size() < 1) return -6;
                stringword = stringword.substr(0, stringword.size() - 1);
                if (stringword[0] != '\"' || stringword[stringword.size() - 1] != '\"') {
                    // @ Error String format error
                    return -6;
                }
                line_address = line_address + stringword.size() - 2 - 1;
                ...
                if (num_temp > 65535 || num_temp < 1) {
                    // @ Error Too large or too small value  @ BLKW
                    return -7;
                }
                line_address = line_address + num_temp;
            } else {
        ...
        if (IsLC3Command(word) != -1 || IsLC3TrapRoutine(word) != -1 || word == "") {
            ...
            if (word == ".BLKW") {
                ...
                label_map.AddLabel(label_name, value_tp(vAddress, line_address - 1));
                line_address = num_temp - 1 + line_address;
            }
            if (word == ".STRINGZ") {
                // modify label map
                // modify line address
    ...
    for (int line_index = 0; line_index < file_content.size(); ++line_index) {
        ...
        if (file_tag[line_index] == lPseudo) {
            ...
            if (word == ".FILL") {
                ...
                std::string HexNULLINE(4,'0');
                std::string NULLINE(16,'0');

                for(int i = 0; i < num_temp; i++) {
                    if(gIsHexMode) output_file << HexNULLINE << std::endl;
                    else output_file << NULLINE << std::endl;
                }
                ...
                // TO BE DONE
                std::string word;
                std::string stringz_str;
                while (line_stringstream >> word) {
                    stringz_str = stringz_str + word + " ";
                }
                stringz_str = stringz_str.substr(1, stringz_str.size() - 3);
                int length = stringz_str.size();
                std::string output_line;

                for(int i = 0; i < length; i++) {
                output_line = NumberToAssemble((int) stringz_str[i]);
        ...
        if (file_tag[line_index] == lOperation) {
            ...
            // TO BE DONE
            auto commapos = parameter_str.find(",");
            for(; commapos != std::string::npos ; commapos = parameter_str.find(",")) {
                parameter_str[commapos] = ' ';
            }
